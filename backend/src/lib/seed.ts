...
// ── Self-invoking guard ────────────────────────────────────────────────────
// Ensures the demo accounts exist on backend startup. seedIfEmpty() short-
// circuits when a user already exists, so this is safe to run on every boot.
// Any failure is logged but never crashes the process.
