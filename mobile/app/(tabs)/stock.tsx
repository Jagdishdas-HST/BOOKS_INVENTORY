          ...
          <View className="gap-sm">
            {movements.map((m) => (
              <View key={m.id} className="flex-row items-center rounded-xl bg-white border border-stone-200 p-md">
                <View className="w-10 h-10 rounded-full bg-amber-100 items-center justify-center mr-sm">
                  <ArrowRightLeft size={16} color="#b45309" />
                </View>
                <View className="flex-1">
                  <Text className="text-stone-900 font-semibold" numberOfLines={1}>{m.bookTitle}</Text>
                  <Text className="text-stone-500 text-xs">→ {m.distributorName} · {format(new Date(m.createdAt), "d MMM, h:mm a")}</Text>
                </View>
                <Text className="text-amber-700 font-extrabold text-lg">+{m.quantity}</Text>
              </View>
            ))}
          </View>
        )}
